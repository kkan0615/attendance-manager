<template>
  ...
  >
    ...
    >
      ...
      >
        <q-img
          v-if="imgToUrl || currentStore.CurrentUser.img"
          class="profile-image"
          fit="cover"
          :src="imgToUrl || currentStore.CurrentUser.img"
        />
        <span
          v-else
          class="tw-text-2xl"
...
const onSubmitForm = async () => {
  try {
    if (img.value) {
      /* Upload */
      userStore.uploadUser({
        id: currentStore.CurrentUser.id,
        file: img.value
      })
    }
    /* Update */
...
const onClickDeleteBtn = () => {
  ...
  $q.dialog({
    ...
    if (data === words) {
      try {
        /* delete */
        await userStore.deleteUser(currentStore.CurrentUser.id)
...
const initData = async () => {
  try {
    name.value = currentStore.CurrentUser.name
    nickname.value = currentStore.CurrentUser.nickname
    email.value = currentStore.CurrentUser.email
    description.value = currentStore.CurrentUser.description || ''
    createdAt.value = dayjs(currentStore.CurrentUser.createdAt).format('ll')
    updatedAt.value = dayjs(currentStore.CurrentUser.updatedAt).format('ll')
  } catch (e) {
    console.error(e)
  }
}
